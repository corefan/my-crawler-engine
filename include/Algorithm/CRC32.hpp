...
namespace algorithm
{
	namespace crc
	{
		...
		public:
			...
			inline crc32& add(void const *pbuf, unsigned long long len) 
			{
				unsigned char *buffer = (unsigned char *)pbuf;
				while(len--)
                    m_crc = (m_crc >> 8) ^ detail::Crc32Table[(m_crc & 0xFF) ^ *buffer++];
				return *this;
			}
		...
		inline ULONG cac_crc32(void const *pbuf, unsigned long long dwSize)
		{
