...
namespace tut
{
    namespace detail
    {
        ...
        struct fpt_traits
        {
            ...
            typedef typename If<std::numeric_limits<T>::is_integer, double, T>::type Result;
        ...
        template<typename T>
        const typename fpt_traits<T>::StdNumericLimitsNotAvailable fpt_traits<T>::static_check[ std::numeric_limits<T>::is_specialized ] = { {} };
        ...
        bool CheckTolerance(T actual, T expected, U fraction)
        {
            typename fpt_traits<T>::Result diff = fpt_traits<T>::div( fpt_traits<T>::abs( expected - actual ), fpt_traits<T>::abs( expected ) );
    ...
    template<typename T, typename U>
    void EnsureClose(const char* msg, const T& actual, const T& expected, const U& tolerance )
    {
        typedef detail::fpt_traits<U> Traits;

        typename Traits::Result fraction = Traits::div( Traits::abs(static_cast<typename Traits::Result>(tolerance)), static_cast<typename Traits::Result>(100) );
      
		if( !detail::CheckTolerance(actual, expected, fraction) )
        {
    ...
    void EnsureClose(const T& actual, const T& expected, const Tolerance& tolerance )
    {
        EnsureClose( 0, actual, expected, tolerance );
    }


	// -----------------------------------------------------------------------------
	// function EnsureCloseFraction

    template<typename T, typename U>
    void EnsureCloseFraction(const char* msg, const T& actual, const T& expected, const U& fraction)
    {
        ...
        typedef typename detail::If<std::numeric_limits<U>::is_integer, double, U>::type Tolerance;
    ...
    template<typename T>
    void EnsureCloseFraction( const char* msg, const T& actual, const T& expected, const int& tolerance )
    {
        EnsureClose(msg, actual, expected, double(tolerance));
    }

    template< typename T, typename Tolerance>
    void EnsureCloseFraction(const T& actual, const T& expected, const Tolerance& fraction)
    {
        EnsureCloseFraction( 0, actual, expected, fraction );
    }
