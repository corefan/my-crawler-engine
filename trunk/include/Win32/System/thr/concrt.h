...
///     the time it is invoked, or if the affinity specified results in an empty set of resources.
...
namespace details
{
    ...
    // Stubs to allow the headers to access runtime functionality for tailored apps.
    class _Context
    {
    public:
    ...
    {
    public:
        _CRTIMP static void _cdecl _ScheduleTask(TaskProc _Proc, void * _Data);
    ...
    struct _Subatomic_impl<4> {
        ...
        static void _StoreWithRelease(volatile _Ty& _Location, _Ty _Rhs) {
            // A volatile write has release semantics as far as the compiler is concerned. In addition, on ARM,
        ...
        static _Ty _LoadWithAquire(volatile _Ty& _Location) {
            // A volatile read has acquire semantics as far as the compiler is concerned. In addition, on ARM,
    ...
    struct _Subatomic_impl<8> {
        template <typename _Ty>
        static void _StoreWithRelease(volatile _Ty& _Location, _Ty _Rhs) {
            // A volatile write has release semantics as far as the compiler is concerned.
            _Location = _Rhs;
        }

        template <typename _Ty>
        static _Ty _LoadWithAquire(volatile _Ty& _Location) {
            // A volatile read has acquire semantics as far as the compiler is concerned.
            return _Location;
        }
    ...
    ///     in place due to the set_task_execution_resources method.
    /// </summary>
    unsigned int _CRTIMP _cdecl _GetConcurrency();
...
/// <summary>
///     This class describes an exception that is thrown due to failure to create a worker execution context in the Concurrency Runtime.
/// </summary>
/// <remarks>
...
///     fails.  Execution contexts can be either threads or UMS threads that execute tasks in the Concurrency Runtime. The error code
///     which would normally be returned from a call to the Win32 method <c>GetLastError</c> is converted to a value of type <c>HRESULT</c>
///     and can be retrieved via the base class method <c>get_error_code</c>.
/// </remarks>
/**/
...
public:
    ...
    /// </param>
    /**/
    _CRTIMP scheduler_worker_creation_error(const char * _Message, HRESULT _Hresult) throw();
...
{
    /// <summary>
    ///     This class describes an exception that is thrown by the PPL tasks layer in order to force the current task
    ///     to cancel.
    /// </summary>
    /// <remarks>
    ///     Users should never see this exception as it is caught internally by the runtime and passed up to the PPL
    ///     tasks layer.
    /// </remarks>
    /**/
    class _Cancel_current_ppl_task : public std::exception
    {
    public:
        /// <summary>
        ///     Constructs a <c>_Cancel_current_ppl_task</c> object.
        /// </summary>
        /// <param name="_Message">
        ///     A descriptive message of the error.
        /// </param>
        /**/
        explicit _CRTIMP _Cancel_current_ppl_task(const char * _Message) throw();

        /// <summary>
        ///     Constructs a <c>_Cancel_current_ppl_task</c> object.
        /// </summary>
        /**/
        _CRTIMP _Cancel_current_ppl_task() throw();
    };
}
...
public:
    ...
    /// <summary>
    ///     Constructs a location representing the system as a whole.
    /// </summary>
    /**/
    location() :
        _M_type(_System),
    ...
    /// </returns>
    /**/
    _CRTIMP static location from_numa_node(unsigned short _NumaNodeNumber);

    /// <summary>
    ///     Returns a location representing the most specific place the calling thread is executing upon.
    /// </summary>
    /// <returns>
    ///     A location representing the most specific place the calling thread is executing upon.
    /// </returns>
    /**/
    _CRTIMP static location current();
    ...
    /// <summary>
    ///     Returns whether two locations are not identical.
    /// </summary>
    /**/
    bool operator!=(const location& _Rhs) const
    {
    ...
    /// <summary>
    ///     Returns a location representing the scheduling node that the calling thread is executing upon.
    /// </summary>
    /**/
    _CRTIMP static location _Current_node();
    ...
    /// </summary>
    /**/
    location(_Type _LocationType, unsigned int _Id, unsigned int _BindingId = 0, void *_PBinding = NULL);
...
private:
    ...
    // The specific binding to a scheduler.  (e.g.: A specific virtual processor for something like location::current() )
...
protected:
    ...
    /// </param>
    /**/
    void operator delete(void * _PObject, int, const char *, int)
    {
        ::operator delete(_PObject); 
    }
};
...
enum SchedulerType
{
    ...
    /// <summary>
    ///     This scheduler type has been deprecated in the Concurrency Runtime in Visual Studio 2012.
    /// </summary>
    /**/
...
public:
    ...
    ///     in a schedule group of the runtime's choosing.  The version that takes the parameter <paramref name="_Placement"/> causes the task
    ...
    /// <seealso cref="location Class"/>
    /**/
    _CRTIMP static void __cdecl ScheduleTask(TaskProc _Proc, void * _Data, location& _Placement);
    ...
    /// </remarks>
    /**/
    _CRTIMP static bool IsAvailableLocation(const location& _Placement);
};
...
namespace details
{
    ...
    // _PPLTaskChore is a special type of _UnrealizedChore. It is used specifically to execute PPL tasks,
    // which need to execute in task collections but must handle the destruction of the executing task
    // collection when either they finish running, or the _TaskCollection::_ScheduleCollectionDeletingTask
    // method has completed, or when the task goes out of scope whichever happens last.
    class _PPLTaskChore : public Concurrency::details::_UnrealizedChore
    {
    public:
        _PPLTaskChore() : _M_collectionRefCount(2)
        {
            _SetRuntimeOwnsLifetime(false);
        }

        // The internal wrapper around invocation of stolen structured chores.
        __declspec(noinline)
        static void __cdecl _DeletingChoreWrapper(_UnrealizedChore * _PChore);

        // The _PPLTaskChore keeps a ref count on its executing collection, when
        // this ref count hits zero, the task collection needs to be destructed.
        volatile long _M_collectionRefCount;
    };
    ...
    protected:
        ...
        friend class Concurrency::details::_PPLTaskChore;
    ...
    public:
        ...
        /// <summary>
        ///     Schedules a chore for execution that will delete its enclosing task group when it finishes execution.  The chore is pushed onto 
        ///     the associated workstealing queue, and will be executed in a LIFO order. The tasks scheduled into a _TaskCollection are scheduled 
        ///     into the current scheduler.
        ///     Note that the specified chore may only be scheduled upon a single task collection at a given time.  Any attempt to schedule the same
        ///     chore multiple times on one or more task collections will result in an invalid_multiple_scheduling exception.  
        ///     This scheduling method is used specifically to execute first-class tasks.  First-class tasks have a different lifetime behavior
        ///     than normal task collections.  Specifically, when the task completes, the underlying task collection must self-delete.
        /// </summary>
        /// <param name="_PChore">
        ///     The new unrealized chore to schedule
        /// </param>
        /**/
        _CRTIMP void _ScheduleCollectionDeletingTask(_PPLTaskChore * _PChore);

        /// <summary>
        ///     A cancellation friendly wrapper with which to execute a PPL task if one exists and then
        ///     waits for the chore running in the _TaskCollection to finish (normally or abnormally). This method is analogous
        ///     to the _RunAndWait() function, but is used specifically to execute PPL tasks, which have slightly different
        ...
        /// </returns>
        /**/
        _CRTIMP _TaskCollectionStatus __stdcall _RunAndWaitPPLTask();
    ...
    struct _Beacon_reference
    {
        ...
        volatile bool _M_flag;
    };

    /// <summary>
    ///     A cancellation beacon is a flag which can be polled in an inlinable fashion via the is_signaled method in lieu of polling on
    ///     the more expensive non inlinable is_current_task_group_canceling method.
    /// </summary>
    /// <remarks>
    ///     Cancellation beacons may only be used in the same way as structured_task_group and _StructuredTaskCollection.  They are intended
    ...
    public:
        ...
        bool _Is_cancel_signaled() const
        {
            return (_M_pRef->_M_flag);
        }
